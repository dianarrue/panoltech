...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idprestamo)){
			$rspta=$prestamo->insertar($idherramienta,$idestudiante,$fecha_prestamo,$fecha_devolucion,$cantidad,$observacion);
			echo $rspta ? "Prestamo registrado" : "Prestamo no se pudo registrar";
		}
		else {
			$rspta=$prestamo->editar($idprestamo,$idherramienta,$idestudiante,$fecha_prestamo,$fecha_devolucion,$cantidad,$observacion);
	...
	case 'anular':
		$rspta=$prestamo->anular($idprestamo);
 		echo $rspta ? "Herramienta devuelto" : "El herramienta esta prestado";
	break;
	...
	case "SelectHerramienta":
        require_once "../modelos/Herramienta.php";
        $herramienta = new $herramientas();
        $rspta = $herramienta->select();
        while ($reg = $rspta->fetch_object())
        {
                echo '<option value=' . $reg->idherramienta . '>' . $reg->titulo . '</option>';
            }   
    break;
